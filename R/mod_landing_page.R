...
mod_landing_page_ui <- function(id) {
  ...
  page_fillable(card(
    ...
    card_body(
      ...
      div(
        tags$ol(
          tags$li("Enter your details in the 'Patient Details' tab."),
          tags$li(
            "Click ",
            span("Start", style = "font-style: bold;"),
            "when you've said each word"
          ),
          tags$li("You'll be randomly assigned to Group A or Group B"),
          tags$ol(
            tags$li(
              "Group A: Stroop Effect (",
          ...
          tags$li("Click", span("Stop", style = "font-style:bold;"),  "when you've said each word"),
        )
      ),
      p("Click \"Stroop Test\" when you're ready to begin")
